...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if(ESX.PlayerData.job ~= nil and IsLawyer()) then
            ...
            for k,v in pairs(Config.Zones) do
                ...
                if(GetDistanceBetweenCoords(v.Pos.x, v.Pos.y, v.Pos.z,  coords, true) < v.Size.x) then 
                    isInMarker = true
                    currentZone = k
                 end
            end
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction then
            ...
            if (IsControlJustReleased(0, 38) and IsLawyer()) then
        ...
        if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'lawyer' then
            OpenActionsMenu()
        end
    end
end)
...
function OpenJobMenu()
    ...
    if(IsBoss() and ESX.PlayerData.job.name=='lawyer')then
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lawyer_actions', {
        ...
        elements = elements
            ...
            CloseJobMenu(menu)
            TriggerServerEvent('esx_lawyerjob:onduty')
...
--Check if lawyer
function IsLawyer()
    return ESX.PlayerData.job.name == 'lawyer' or ESX.PlayerData.job.name == 'offlawyer'
end